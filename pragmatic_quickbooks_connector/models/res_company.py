...
class ResCompany(models.Model):
    ...
    def import_chart_of_accounts(self):
        ...
        data = requests.request('GET', url, headers=url_str.get('headers'))
        if data:
            _logger.info("Charts of accounts data is ----------------> {}".format(data.text))
    ...
    def import_payment(self):
        ...
        if data:
            _logger.info("Data for importing customer payments in odoo is ---> {}".format(data))
                    ...
                    if parsed_data.get('QueryResponse') and parsed_data.get('QueryResponse').get('Term'):
                        for term in parsed_data.get('QueryResponse').get('Term'):
                            ...
                            if not exists:
                                ...
                                vals.update({'line_ids': [(0, 0, {'value': 'balance', 'days': term.get('DueDays')})]})
                            ...
                            if "CustomerRef" in cust and cust.get('CustomerRef').get('value'):
                                ...
                                if not sale_order:
                                    ...
                                    if res_partner:
                                        ...
                                        if cust.get('TxnDate'):
                                            dict_s['date_order'] = cust.get('TxnDate')
        ...
        for inv in invoices:
            if inv.partner_id.customer_rank:
                if inv.state == 'open' and inv.qbo_invoice_id:
                    ...
                    # print(inv.move_type,'_________________', inv.qbo_invoice_id)
                    if inv.qbo_invoice_id:
                        if inv.move_type != 'out_refund' or inv.move_type != 'in_refund':
                            inv.export_to_qbo()
                        else:
                            _logger.info(_("This '%s' can not be exported as this is refund type." % inv.name))
